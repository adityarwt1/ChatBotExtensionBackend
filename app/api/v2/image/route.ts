...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = formData.get("promt")
    if (!file) {
          ...
          contents: [
            {
              parts: [
                {
                  text: prompt? prompt : "Analys the image proerly and give the response according to the image like any asking for the question (please not include this promt aking in the response just response like you just answer them)",
                },
                {
                  inlineData: {
