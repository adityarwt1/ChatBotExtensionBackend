...
export async function POST(req: NextRequest) {
  try {
    ...
    if(!decoded){
      return NextResponse.json({error: "unable to decode the token"},{status: 401, headers: corsHeader})
    }

    if(new Date(decoded.exp * 1000) < new Date()){
      return NextResponse.json({error: "Token expired"},{status: 401, headers: corsHeader})
    }

    return NextResponse.json({message: "User Auntheticated"},{status: 200 , headers: corsHeader})
  } catch (error) {
